...
import com.spe.poll.util.ModelMapper;
...
public class PollService {
    ...
    public PagedResponse<PollResponse> getAllPolls(User currentUser, int page, int size) {
...
//    }).getContent();
    ...
    public PagedResponse<PollResponse> getPollsCreatedBy(String username, User currentUser, int page, int size) {
        validatePageNumberAndSize(page, size);
        ...
        Pageable pageable = PageRequest.of(page, size, Sort.Direction.DESC, "createdAt");
        Page<Poll> polls = pollRepository.findByCreatedBy((long) user.getId(), pageable);
...
//    }).getContent();
    ...
    public PagedResponse<PollResponse> getPollsVotedBy(String username, User currentUser, int page, int size) {
        validatePageNumberAndSize(page, size);
        ...
        Pageable pageable = PageRequest.of(page, size, Sort.Direction.DESC, "createdAt");
        Page<Long> userVotedPollIds = voteRepository.findVotedPollIdsByUserId((long) user.getId(), pageable);
...
//    }).collect(Collectors.toList());
    ...
    public PollResponse getPollById(Long pollId, User currentUser) {
        ...
        Vote userVote = null;
        if(currentUser != null) {
            userVote = voteRepository.findByUserIdAndPollId((long) currentUser.getId(), pollId);
        }
    ...
    public PollResponse castVoteAndGetUpdatedPoll(User currentUser, Long pollId, VoteRequest voteRequest) {
    ...
    private Map<Long, Long> getPollUserVoteMap(User currentUser, List<Long> pollIds) {
        // Retrieve Votes done by the logged in user to the given pollIds
        Map<Long, Long> pollUserVoteMap = null;
        if(currentUser != null) {
            List<Vote> userVotes = voteRepository.findByUserIdAndPollIdIn((long)currentUser.getId(), pollIds);
    ...
    Map<Long, User> getPollCreatorMap(List<Poll> polls) {
        ...
        for (User creator : creators) {
            if (creatorMap.put((long) creator.getId(), creator) != null) {
