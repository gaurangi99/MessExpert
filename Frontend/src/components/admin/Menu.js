...
const Menu = () => {
  ...
  const saveImage = (event) => {
    ...
    console.log(image);
    imageService.addMenu(image).then(res=>{
      setSpinner(false);
      if(res.status===200){
  ...
  <div>
  <h2 className="text-center m-4">Current Mess Menu</h2>
  <div style={{ display: 'flex', justifyContent: 'center' }}>
  <ImageComponent fileName="IIITB_MENU"/>
  </div>
  </div>
  </div>
  </div>
  <div className="row">
  <div className="col-md-12 offset-md-3 border rounded p-4 mt-2 shadow">
                  <h2 className="text-center m-4">Upload Mess Menu</h2>
